<?php
    ...
    $pdf->AddPage();
    $pdf->SetTitle('Laporan Data Matakuliah FTI');
    ...
    $pdf->Line(1, 3, 20, 3);
    $pdf->Ln();
    $pdf->Cell(19, 1, 'Laporan Data Matakuliah FTI 2020', 0, 1, 'C');
    $pdf->Ln();
    ...
    $pdf->Cell(3,1,'',0,0,'');
    $pdf->Cell(4,1,'Kode',1,0,'C',true);
    $pdf->Cell(5,1,'Matakuliah',1,0,'C',true);
    $pdf->Cell(3,1,'SKS',1,1,'C',true);
    $pdf->SetFont('Arial','',11);
    $pdf->SetFillColor(40, 200, 170);
    $q  = mysqli_query($koneksi, "Select * from matakuliah");
    while($row = mysqli_fetch_array($q)){
        $pdf->Cell(3,1,'',0,0,'');
        $pdf->Cell(4, 0.7, $row[0], 1, 0, 'C', true);
        $pdf->Cell(5, 0.7, $row[1], 1, 0, 'C', true);
        $pdf->Cell(3, 0.7, $row[2], 1, 1, 'C', true);
    }
    $pdf->Ln(2);
